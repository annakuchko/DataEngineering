...
SRC = "host='localhost' port='5434' dbname='my_database' user='root' password='postgres'" 
TRG = "host='localhost' port='5433' dbname='my_database' user='root' password='postgres'"
...
def copy_data(source_conn_string=SRC, target_conn_string=TRG, verbose=False):
    check_tables(source_conn_string, target_conn_string, verbose)
    with psycopg2.connect(source_conn_string) as \
        conn_source, conn_source.cursor() as cursor_source:
        source_tables = get_tables(cursor_source)
    for tbl in source_tables:
